...
"""Fine-tuning script InteractDiffusion for Stable Diffusion XL for text2image."""
...
from interactdiffusion_xl.hicodet import HICODetDataset
from interactdiffusion_xl.interactdiffusion_unet_2d_condition import InteractDiffusionUNet2DConditionModel
from interactdiffusion_xl.pipeline_interactdiffusion_sdxl import StableDiffusionXLInteractDiffusionPipeline
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The path to weights to load the pretrained GLIGEN weights in UNet.",
    )
    parser.add_argument(
        "--data_path",
        type=str,
...
def main(args):
    ...
    # unet = InteractDiffusionUNet2DConditionModel.from_pretrained(
    #     args.gligen_unet_path, subfolder="unet",
    #     revision=args.revision,
    #     variant=args.variant,
    ...
    # )  
    unet = InteractDiffusionUNet2DConditionModel.from_config(
        InteractDiffusionUNet2DConditionModel.load_config(args.gligen_unet_path))
    state_dict_files = sorted([f for f in os.listdir(args.gligen_unet_path) if f.endswith(".safetensors")])
    combined_state_dict = {}
    for file_name in state_dict_files:
        file_path = os.path.join(args.gligen_unet_path, file_name)
        print(f"file_path {file_path}")
        part_state_dict = load_file(file_path)  # Load using safetensors
        combined_state_dict.update(part_state_dict)  # Add each part to the combined dictionary
    ...
    interact_diffusion_dict = {}
    for (target, weight) in list_to_copy:
        interact_diffusion_dict[target] = combined_state_dict[weight]
    dict_to_init = {
        "position_net.interaction_embedding.emb.weight":
            unet.position_net.interaction_embedding.emb.weight,
        "position_net.position_embedding.emb.weight":
            unet.position_net.position_embedding.emb.weight
    }
    for key in dict_to_init:
        interact_diffusion_dict[key] = dict_to_init[key]
    combined_state_dict.update(interact_diffusion_dict)
    
    unet.load_state_dict(combined_state_dict)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            ...
            for _ in range(len(models)):
                ...
                load_model = InteractDiffusionUNet2DConditionModel.from_pretrained(input_dir, subfolder="unet")
    ...
    train_dataset = HICODetDataset(path=args.data_path,
                                   prob_use_caption=args.prob_use_caption,
                                   max_boxes_per_data=32,
                                   embedding_len=unet_config.cross_attention_dim)
    
        
    def collate_fn(examples):
        collated_examples = {}
        
        for key in ['latents', 'subject_boxes', 'object_boxes', 'text_masks',
        ...
        collated_examples['latents'] = collated_examples['latents'].squeeze(1)
        collated_examples['latents'] = collated_examples['latents'].to(memory_format=torch.contiguous_format)

        for key in ['id', 'caption', 'original_sizes', 'crop_top_lefts']:
    ...
    while global_step < args.max_train_steps:
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            subject_boxes = batch['subject_boxes'].to(accelerator.device, dtype=weight_dtype)
            object_boxes = batch['object_boxes'].to(accelerator.device, dtype=weight_dtype)
            subject_text_embeddings = batch['subject_text_embeddings'].to(accelerator.device, dtype=weight_dtype)
            object_text_embeddings = batch['object_text_embeddings'].to(accelerator.device, dtype=weight_dtype)
            action_text_embeddings = batch['action_text_embeddings'].to(accelerator.device, dtype=weight_dtype)
            ...
            if accelerator.is_main_process and accelerator.sync_gradients:
                if global_step % args.validation_steps == 0:
                    ...
                    pipeline = StableDiffusionXLInteractDiffusionPipeline.from_pretrained(
                        args.pretrained_model_name_or_path,
                        vae=vae,
                    ...
                    args.validation_prompts = ["a person is feeding a cat"]
                    ...
                    with autocast_ctx:
                        images = [
                            pipeline(args.validation_prompts[i], generator=generator, num_inference_steps=25,
                                    interactdiffusion_subject_phrases=interactdiffusion_subject_phrases[i],
                                    interactdiffusion_object_phrases=interactdiffusion_object_phrases[i],
                                    interactdiffusion_action_phrases=interactdiffusion_action_phrases[i],
                                    interactdiffusion_subject_boxes=interactdiffusion_subject_boxes[i],
                                    interactdiffusion_object_boxes=interactdiffusion_object_boxes[i],
                                    gligen_scheduled_sampling_beta=1,
                                    height=args.resolution, width=args.resolution).images[0]
                            for i in range(len(args.validation_prompts))
                        ]
                    
                    for image, _sbj_boxes, _obj_boxes in zip(images, interactdiffusion_subject_boxes, interactdiffusion_object_boxes):
                        [draw_obj(image, box) for box in (_sbj_boxes+_obj_boxes)]
