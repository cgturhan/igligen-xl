...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="./data", help="Root folder containing image classes")
    ...
    train_shards = []
    for city, images in filtered_files.items():
        train_shards.append({
            "city": city,
            "images": images
        })

    dataset = ECPDatasetSDXL(data_path=args.data_path, train_shards=train_shards, prob_use_caption=0.5, prob_use_boxes=0.9, box_confidence_th=0.25, batch_size=1, transform=None, shard_shuffle_seed=None, ddp_rank=0, num_ddp_processes=1)
    dataset.__iter__()
